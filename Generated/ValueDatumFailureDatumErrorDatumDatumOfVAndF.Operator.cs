    ...
    public ValueDatumFailureDatumErrorDatumDatum<OV, OF> Then<OV, OF>(
        ...
        try {
            return (this.Mode) switch {
                ValueDatumFailureDatumErrorDatumMode.Value => (funcValueDatum is not null) ? funcValueDatum(this.Value, defaultValue) : defaultValue,
                ValueDatumFailureDatumErrorDatumMode.Failure => (funcFailureDatum is not null) ? funcFailureDatum(this.Failure, defaultValue) : defaultValue,
                ValueDatumFailureDatumErrorDatumMode.Error => (funcErrorDatum is not null) ? funcErrorDatum(this.Error, defaultValue) : this.Error,
            _ => defaultValue
            };
